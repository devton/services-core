...
RSpec.describe Project, type: :model do
  ...
  describe 'associations' do
    ...
    it { is_expected.to have_many :project_integrations }
  ...
  describe '.with_state' do
    ...
    context 'when state is a list' do
      let(:project_state) { %w[online failed] }
      before do
        4.times { create(:project, state: 'online') }
  ...
  describe '.visible' do
    before do
      %i[draft rejected deleted].each do |state|
  ...
  describe '.state_names' do
    let(:states) { %i[draft rejected online successful waiting_funds failed deleted] }
  ...
  describe '.to_finish' do
    before do
      ...
      expect(Project).to receive(:with_states).with(%w[online waiting_funds]).and_call_original
    end
